...
const manifest = require('./source-cover-manifest.json')
const outputRoot = path.resolve(process.argv[2] || path.join(os.tmpdir(), 'today-eat-source-covers'))
...
async function download(item, index) {
  ...
  if (fs.existsSync(outputPath) && fs.statSync(outputPath).size >= 1000) {
    process.stdout.write(`\r成品封面 ${index + 1}/${manifest.length}`)
    return
  }
